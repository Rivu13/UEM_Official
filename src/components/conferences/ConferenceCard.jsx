...
export default function ConferenceCard({ conference }) {
  return (
    <div className="flex flex-col overflow-hidden rounded-xl border border-slate-200 bg-white shadow-sm transition-shadow duration-300 hover:shadow-md">
      <Link to={`/conferences/${conference.slug}`} className="aspect-[16/9] w-full overflow-hidden bg-slate-100">
        <img
          src={conference.gallery[0]}
      ...
      <div className="flex flex-1 flex-col p-5">
        ...
        <p className="mt-2.5 flex-1 text-sm leading-relaxed text-slate-600">{conference.excerpt}</p>
        ...
        <div className="mt-4 flex flex-wrap items-center justify-between gap-x-3 gap-y-1 border-t border-slate-100 pt-3 text-xs">
          ...
          <span className="text-right text-blue-700">{conference.tags.join(", ")}</span>
        </div>
      </div>
    </div>
  )
